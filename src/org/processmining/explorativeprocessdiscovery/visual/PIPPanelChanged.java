 ...
 *
 */
public class PIPPanelChanged extends JPanel implements MouseListener, MouseMotionListener {
	...
	private final ProjectionVisPanelChanged view;


	public PIPPanelChanged(ProjectionVisPanelChanged view) {
	...
	public synchronized void initializeImage() {
		Dimension size = miniembeddedconfig.getZoom().getPreferredSize();
		if (size.getWidth() > 0 && size.getHeight() > 0 && getWidth() > 0 && getHeight() > 0) {
			...
			BufferedImage image = (BufferedImage) miniembeddedconfig.getZoom().createImage(getWidth(), getHeight());
			Graphics2D g2ds = image.createGraphics();

			g2ds.setColor(miniembeddedconfig.getZoom().getBackground());
	...
	public double getVisWidth() {
		return miniembeddedconfig.getZoom().getSize().getWidth();
	}

	public double getVisHeight() {
		return miniembeddedconfig.getZoom().getSize().getHeight();
	}

...
public class ZoomPanel extends JPanel {
	...
	protected final ProjectionVisPanelChanged mainView;

	public ZoomPanel(ProjectionVisPanelChanged mainPanel, PIPPanelChanged pip, int maximumZoom) {
		...
		slider.setValue((int) Math.floor(100 * miniembeddedconfig.getZoom().getScale()));
	}

	private void update() {
		int value = this.slider.getValue();
		this.sliderValue.setText(value + "%");
		miniembeddedconfig.getZoom().setScale((double)(slider.getValue()) / 100);
	}
	...
	public void computeFitScale() {
		...
		double scale = miniembeddedconfig.getZoom().getScale();
		Dimension b = miniembeddedconfig.getZoom().getPreferredSize();

...
public class FitnessVisualizer {
	...
	@Plugin(name = "@0 Fitness Visualizer", level = PluginLevel.PeerReviewed, returnLabels = {
	...
	public JComponent visualize(PluginContext context, ResultUnitList ret) {
		if (miniprojectVisualconfig.getVersionFlag() == 0) {
			...
			miniprojectVisualconfig.setType(0);
			return new MainProjectionVisPanelFull(context, results, 0);
			...
			return new MainProjectionVisPanelLite(context, results, 0);
		} else if (miniprojectVisualconfig.getVersionFlag() == 2) {
			List<Result> results = new ArrayList<Result>();
			for (ResultUnit i : miniprojectVisualconfig.getBestRouteResult().get(0)) {
				Result r = new Result(i.name, i.fitness, i.precision, i.f1, i.eventcount, i.activitycount, i.subset);
				results.add(r);
			}
			Comparator<Result> comparatorAge = new Comparator<Result>() {
			...
			results.sort(comparatorAge);
			miniprojectVisualconfig.setType(0);
			return new MainProjectionVisPanelBestRoute(context, results, 0);
		} else {
			return null;			
		}
	}

...
public class PrecisionVisualizer {
	MainVisualConfig miniprojectVisualconfig = new MainVisualConfig();

	@Plugin(name = "@0 Precision Visualizer", level = PluginLevel.PeerReviewed, returnLabels = {
	...
	public JComponent visualize(PluginContext context, ResultUnitList ret) {
		if (miniprojectVisualconfig.getVersionFlag() == 0) {
			...
			miniprojectVisualconfig.setType(1);
			return new MainProjectionVisPanelFull(context, results, 1);
			...
			return new MainProjectionVisPanelLite(context, results, 1);
		} else if (miniprojectVisualconfig.getVersionFlag() == 2) {
			List<Result> results = new ArrayList<Result>();
			for (ResultUnit i : miniprojectVisualconfig.getBestRouteResult().get(1)) {
				Result r = new Result(i.name, i.fitness, i.precision, i.f1, i.eventcount, i.activitycount, i.subset);
				results.add(r);
			}
			Comparator<Result> comparatorAge = new Comparator<Result>() {
			...
			results.sort(comparatorAge);
			miniprojectVisualconfig.setType(1);
			return new MainProjectionVisPanelBestRoute(context, results, 1);
		} else {
			return null;			
		}
	}
